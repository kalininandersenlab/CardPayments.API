...
namespace CardPaymentAPI.Models
{
    public class Card
    {
        ...
        public async Task<bool> IsCardAuthorized(decimal amount)
        {
            if (!IsActive)
            {
            ...
            bool isFraudulent = CheckFraudulentTransactions(amount);
            if (isFraudulent)
            {
        ...
        private bool CheckFraudulentTransactions(decimal amount)
        {
            var recentTransactions = Transactions
                .Where(t => t.Card.Id == Id && t.Timestamp > DateTime.UtcNow.AddSeconds(-10));
